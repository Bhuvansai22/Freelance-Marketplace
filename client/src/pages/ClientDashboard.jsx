...
import { Briefcase, IndianRupee, Clock, Users, ArrowRight, Eye, Check, X, FileText, MessageSquare, Star, Loader2, Award, User, Calendar, Mail, Phone, CheckCircle, ShieldCheck } from 'lucide-react';
...
const ClientDashboard = () => {
  ...
  const handleChatbotFreelancerClick = async (freelancerId, freelancerName) => {
    const found = freelancers.find(f => f._id === freelancerId);
    if (found) {
      handleStartChat(found);
    } else {
      handleStartChat({ _id: freelancerId, name: freelancerName || 'Specialist' });
    }
  };
  ...
  return (
    <div className="min-h-screen relative bg-slate-50 dark:bg-dark-bg transition-colors duration-200 aurora-mesh tech-grid">
            ...
            {isFreelancersLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {freelancers.map((freelancer, idx) => {
                  const isVerified = freelancer.isVerified || false;
                  const showBadges = freelancer.showBadges !== false;
                  const verifiedList = Array.isArray(freelancer.verifiedBadges) ? freelancer.verifiedBadges : [];
                  const visibleList = Array.isArray(freelancer.visibleBadges) ? freelancer.visibleBadges : [];
                  
                  // Clean, case-insensitive badge selector. Falls back to all verified badges by default.
                  const badgesToDisplay = showBadges
                    ? (visibleList.length > 0
                        ? verifiedList.filter(b => visibleList.some(vb => vb.toLowerCase().trim() === b.toLowerCase().trim()))
                        : verifiedList)
                    : [];
                  ...
                  return (
                    ...
                    >
                      <div>
                        ...
                        {/* Earned Badges Row */}
                        {badgesToDisplay.length > 0 && (
                          <div className="flex flex-wrap gap-1 mb-4 animate-fade-in">
                            {badgesToDisplay.map((badge) => {
                              ...
                              return (
                                <span key={badge} className="px-2 py-0.5 bg-amber-500/10 text-amber-600 dark:text-amber-400 rounded-full text-[9px] font-bold border border-amber-500/20 shadow-sm">
                                  {displayLabel}
                                </span>
                              );
                            })}
                        ...
                        {/* Skill Tags */}
                        <div className="flex flex-wrap gap-1.5 mb-6 pt-3 border-t border-slate-100 dark:border-slate-800">
                          {(freelancer.skills?.length > 0 ? freelancer.skills : ['Javascript', 'React', 'Node.js', 'CSS']).map((skill) => (
      ...
      <AIChatbotWidget 
        userRole="client" 
        onFreelancerClick={handleChatbotFreelancerClick} 
      />
    </div>
  );
};
